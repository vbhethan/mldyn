...
class Node2Edge(torch.nn.Module):
    def __init__(self, n_node_features, n_edge_features, hidden_edge_features, out_edge_features):
        super().__init__()
...
class Node2Edge_v2(MessagePassing):
    # This one implements the edge updates with the Pyg MessagePassing class
    def __init__(self, n_node_features, n_edge_features, hidden_edge_features, out_edge_features):
    ...
    def forward(self, edge_index: Adj, x: Tensor, edge_attr: Tensor) -> Tensor:
    ...
    def edge_update(self, x_j: Tensor, x_i: Tensor, edge_attr: Tensor) -> Tensor:
        out = torch.cat([x_i, x_j, edge_attr])
        out = self.mlp(out)
